...
async function loadLeaderboardData() {
    try {
        ...
        const sortBy = document.getElementById('sortBy')?.value || 'xp';

        const response = await fetch(AppConfig.apiUrl(`gamification.php?action=get_leaderboard&period=${periodFilter}&category=${categoryFilter}&sort=${sortBy}`), {
...
function displayTrends() {
    ...
    if (!trendsSection || !leaderboardData || leaderboardData.length === 0) {
        return;
    }

    // Calculate simple trend data
    const topUsers = leaderboardData.slice(0, 5);
    let html = '<div class="row">';

    topUsers.forEach((user, index) => {
        const trendIcon = user.trend === 'up' ? 'fa-arrow-up text-success' :
            user.trend === 'down' ? 'fa-arrow-down text-danger' :
                'fa-minus text-muted';
        ...
        html += `
            <div class="col-md-2 mb-3">
                <div class="trend-card">
                    ...
                    <div class="trend-name">${user.nama || 'Peserta'}</div>
                    <div class="trend-xp">${formatXP(user.total_xp || 0)} XP</div>
